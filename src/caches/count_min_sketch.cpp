...
int32_t CountMinSketch::SingleGroup::Get(uint32_t count_index) {
    static constexpr int32_t kEmpty = 0;
    if(base_.empty()) {
        return kEmpty;
    }
    auto value = base_[count_index].to_ulong();
    return value;
}
...
int32_t CountMinSketch::Estimate(uint32_t hash_val) {
    ...
    for(int32_t index = 0; index < kCmsDepth; ++index) {
        min_val = std::min(groups_[index].Get((hash_val ^ seeds_[index]) & mask_), min_val);
    }
    return min_val;
}
